...
from .copula_survival_functions import resample_is, init_marginals,update_copula_single
...
def update_pn(carry,i):
    vn,log_w,ESS,particle_ind,a_rand,delta,x,logcdf_yn,logpdf_yn,preq_loglik,a,rho_x = carry
    ...
    #update
    logcdf_yn,logpdf_yn= update_copula_B(logcdf_yn,logpdf_yn,u,v_y,logalpha_x,a)
    ...
    carry = vn,log_w,ESS,particle_ind,a_rand,delta,x,logcdf_yn,logpdf_yn,preq_loglik,a,rho_x
    return carry,i
...
def update_pn_loop(key,t,delta,x,a,rho_x,B):
    n = jnp.shape(t)[0]
...
def nll(log_hyperparam,key,t,delta,x,B):
    a = jnp.exp(log_hyperparam[0])
    rho_x = 1/(1+jnp.exp(log_hyperparam[1]))
    *_,preq_loglik= update_pn_loop(key,t,delta,x,a,rho_x,B)
...
def update_ptest(carry,i):
    vn,x,x_test,logcdf_ytest,logpdf_ytest,a,rho_x = carry
    ...
    #update
    logcdf_ytest,logpdf_ytest= update_copula_B(logcdf_ytest,logpdf_ytest,u,v,logalpha_x,a)

    carry = vn,x,x_test,logcdf_ytest,logpdf_ytest,a,rho_x
    return carry,i
...
@jit
def update_ptest_loop(vn,x,y_test,x_test,a,rho_x):
    B = jnp.shape(vn)[0]
    n = jnp.shape(vn)[1]
