...
from .copula_survreg_functions import nll,grad_nll,update_pn_loop,update_ptest_loop
from .sample_survreg_functions import predictive_resample_loop, pr_loop_conv

### Fitting ###
#Compute overhead v_{1:n}, return fit copula object for prediction
def fit_copula_survival(t,delta,x,B,seed = 20,hyperparam_grid = None,B_opt = None,a_init = 1., rho_x_init = 0.9):
    if B_opt is None:
        B_opt = B
    ...
    print('Compiling...')
    start = time.time()
    temp = nll(jnp.log(jnp.array([a_init,1/rho_x_init - 1])),key,t,delta,x,B_opt).block_until_ready()
    if hyperparam_grid is None:
        temp = grad_nll(jnp.log(jnp.array([a_init,1/rho_x_init - 1])),key,t,delta,x,B_opt).block_until_ready()
    temp =update_pn_loop(key,t,delta,x,1.,rho_x_init,B)[0].block_until_ready()
    end = time.time()
    ...
    if hyperparam_grid is None:
        ...
        log_hyperparam_init = jnp.log(jnp.array([a_init,1/rho_x_init - 1]))
        ...
        nll_h = np.zeros(n_h)
        for i in range(n_h):
            a,rho = hyperparam_grid[i]
            nll_h[i] = nll(jnp.log(np.array([a,1/rho - 1])),key,t,delta,x,B_opt)

        a_opt,rho_opt = hyperparam_grid[np.argmin(nll_h)]
    ...
    print('Fitting...')
    start = time.time()
    vn,log_w,ESS,particle_ind,_,_,preq_loglik= update_pn_loop(key,t,delta,x,a_opt,rho_opt,B)
    temp = vn.block_until_ready()
    end = time.time()
    print('Fit time: {}s'.format(round(end-start, 3)))

    copula_survival_obj = namedtuple('copula_survival_obj',['vn','log_w','a_opt','rho_x_opt','x','preq_loglik', 'ESS','particle_ind'])
    return copula_survival_obj(vn,log_w,a_opt,rho_opt,x,preq_loglik, ESS, particle_ind)
...
def predict_copula_survival(copula_survival_obj,y_test,x_test):
    print('Predicting...')
    start = time.time()
    logcdf,logpdf = update_ptest_loop(copula_survival_obj.vn,copula_survival_obj.x,y_test,x_test,copula_survival_obj.a_opt,copula_survival_obj.rho_x_opt)
...
def predictive_resample_survival(copula_survival_obj,y_test,x_test, T_fwdsamples = 2000, seed = 100, resample = True):
    ...
    logcdf_pr, logpdf_pr = predictive_resample_loop(key,logcdf,logpdf,copula_survival_obj.x,x_test,copula_survival_obj.a_opt,copula_survival_obj.rho_x_opt,n,T_fwdsamples)
...
def check_convergence_pr(copula_survival_obj,y_test,x_test,B,T_fwdsamples = 10000, seed = 100):
    ...
    start = time.time()
    n = jnp.shape(copula_survival_obj.vn)[1] #get original data size
    logcdf_pr, logpdf_pr,pdiff,cdiff = pr_loop_conv(key,logcdf[0:B],logpdf[0:B],copula_survival_obj.x,x_test,copula_survival_obj.a_opt,copula_survival_obj.rho_x_opt,n,T_fwdsamples)
    logcdf_pr = logcdf_pr.block_until_ready() #for accurate timing
    end = time.time()
    print('Predictive resampling time: {}s'.format(round(end-start, 3)))
